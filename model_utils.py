...
def prepare_data(df: pd.DataFrame, target_col: str):
    ...
    return X, y, classes, cat_cols, num_cols
...
def evaluate_models(X, y, classes, test_size=0.2, random_state=42, cv_splits=5):
    ...
    models = train_all_models(random_state=random_state)
    ...
    for name, clf in models.items():
        ...
        if hasattr(pipe.named_steps["clf"], "predict_proba"):
            ...
            s = pipe.named_steps["clf"].decision_function(X_test)
...
def run_inference_on_new_data(new_df: pd.DataFrame, fitted_pipeline, label_positive=None, proba_colname="Attrition_Prob"):
    Xnew = new_df.copy()
